...
//	public static final String TAG = "AudioPlayerActivity";
	...
	public void setSentenceList(List<Sentence> list) {
		this.list = list;
		Log.d(TAG, "setSentenceList    list.size()=="+list.size());
	}
	...
	private void init() {	
				...
				Log.d(TAG,"init---mDensity="+mDensity);
				mPaint.setShadowLayer(1.5f * mDensity, 0, 0, Color.WHITE);
...
//                		canvas.translate(0, MarginOfEachSentence);
        			...
        			if(i== index -1){
        				canvas.translate(0, 10*mDensity);
        			}
		...
		//	Utils.printLog(TAG, "lyric count first"+strBuilder.length());
			...
			if (count > 0) {
				for (int j = count; j >= 0; j--) {
					if (isChinese) {
						str = new String(strBuilder.substring(0,
								strBuilder.length()));
						break;
					}
