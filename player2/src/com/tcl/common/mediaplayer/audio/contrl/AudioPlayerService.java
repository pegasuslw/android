...
public class AudioPlayerService extends Service implements
	...
	public void onCreate() {
		...
		intentFilter1.addAction(Intent.ACTION_SCREEN_OFF); ////add here for str 待机再开机黑屏问题  20150901 WJ
		intentFilter1.addAction(CommonConst.TV_PRESSED);
		intentFilter1.addAction(CommonConst.CHANGE_SOURCE_PRESSED);
	   intentFilter1.addAction(CommonConst.HISTORYRECORD);
		intentFilter1.addAction(CommonConst.SOURCE_CHANGEandVOICEASS);
	...
	private BroadcastReceiver mShutDownBroadCastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Intent.ACTION_SHUTDOWN.equalsIgnoreCase(intent.getAction())) {
			...
			}//add here for str 待机再开机黑屏问题  20150901 WJ
			else if(Intent.ACTION_SCREEN_OFF.equalsIgnoreCase(intent.getAction())){
				Utils.printLog(TAG, "str shut down");
				releaseMediaPlayer();
				callBack_onAudioPlayError(CommonConst.HOME_PRESSED_BroadCast);
				...
				releaseMediaPlayer();
				callBack_onAudioPlayError(CommonConst.HOME_PRESSED_BroadCast);
			}else if(CommonConst.HISTORYRECORD.equalsIgnoreCase(intent.getAction())){
				Utils.printLog(TAG, "history record PRESSED");
				releaseMediaPlayer();
			}else if(CommonConst.SOURCE_CHANGEandVOICEASS.equalsIgnoreCase(intent.getAction())){
				Utils.printLog(TAG, "source change or voice assistant PRESSED");
				releaseMediaPlayer();
			}
		}
