...
public class VideoAdvanceDialogActivity extends Activity {
	...
	private TextView sutitle, audiotrack ,subencode_text;
	private LinearLayout sub_Lay, track_Lay ,subencode_Lay;
	private int subNms, trackNms,subencodeNms;
	private int mCurrSub, mCurrTrack ,mCurrSubencode;
	...
	private TextView subtitlename, audiotrackname ,subencodename;
...
//	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_RIGHT) {
			...
			if (sub_Lay.hasFocus()&& isNextSubtitelValid) {// 此时选中的是字幕菜单；
			...
			}else if(subencode_Lay.hasFocus()&& isNextSubtitelValid){   //选中字母编码格式
				if (subNms <= 0) {// 当前视频无字幕，则按键无效；
					return false;
				}
				isNextSubtitelValid = false; //??
				setSubtiteValid();           //??
				if(mCurrSubencode == 0){
					SystemProperties.set("ms.subtitle.language","windows-1256");
					mCurrSubencode = 1;
				}else if(mCurrSubencode == 1){
					SystemProperties.set("ms.subtitle.language","windows-1255");
					mCurrSubencode = 2;
				}
				else if(mCurrSubencode == 2){
					SystemProperties.set("ms.subtitle.language","windows-1250");
					mCurrSubencode = 3;
				}
				else if(mCurrSubencode == 3){
					SystemProperties.set("ms.subtitle.language","0");
					mCurrSubencode = 0;
				}
				if(mVideoContrl != null){
					
					reFreshSubtitleCodeType();
				}
			}
			...
			}else if(subencode_Lay.hasFocus()&& isNextSubtitelValid){
				if (subNms <= 0) {// 当前视频无字幕，则按键无效；
					return false;
				}
				isNextSubtitelValid = false; //??
	...
	private void reFreshSubtitleCodeType(){
		if (mVideoContrl != null && isGetContent && subNms > 0) {
			subencode_Lay.setFocusable(true);
		    String nowencode = SystemProperties.get("ms.subtitle.language", null);
		    if(nowencode.equalsIgnoreCase("windows-1256")){
		    	subencode_text.setText(VideoAdvanceDialogActivity.this
						.getString(R.string.arab_encode));
		    }else if(nowencode.equalsIgnoreCase("windows-1255")){
		    	subencode_text.setText(VideoAdvanceDialogActivity.this
						.getString(R.string.xibolai_encode));
            }else if(nowencode.equalsIgnoreCase("windows-1250")){
                subencode_text.setText(VideoAdvanceDialogActivity.this.getString(R.string.zhongou_encode));
            }
		    else{
		    	subencode_text.setText(VideoAdvanceDialogActivity.this
						.getString(R.string.default_track));
		    }			
			subencode_text.setTextColor(getResources().getColor(R.color.ui_textcolor));
		} else {
			
			subencode_text.setText(VideoAdvanceDialogActivity.this
					.getString(R.string.default_sub));
			subencode_text.setTextColor(getResources().getColor(R.color.text_color_disabled));
			subencode_Lay.setFocusable(false);
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        subencode_Lay = (LinearLayout)findViewById(R.id.subtitledecode);
		subencode_text = (TextView)findViewById(R.id.subtitle_encode_text);
		subencodename = (TextView)findViewById(R.id.subtitle_encode);
		reFreshSubtitleText();
		reFreshTrackText();
		reFreshSubtitleCodeType();		
